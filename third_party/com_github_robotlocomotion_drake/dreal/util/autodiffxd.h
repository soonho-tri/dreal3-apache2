...
class AutoDiffScalar<VectorXd>
 ...
 public:
  ...
  friend inline const AutoDiffScalar<DerType> operator*(
    ...
    return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(a.value() * other,
                                           a.derivatives() * other);
  }
  ...
  inline const AutoDiffScalar<DerType> operator/(
    ...
    return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(
        ...
        has_both_der
            ...
            : has_this_der ? VectorXd(this_der * other.value()) * scale :
...
DRAKE_EIGEN_AUTODIFFXD_DECLARE_GLOBAL_UNARY(
    ...
    return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(sqrtx, x.derivatives() *
                                                      (Scalar(0.5) / sqrtx));)

DRAKE_EIGEN_AUTODIFFXD_DECLARE_GLOBAL_UNARY(
    cos, using std::cos; using std::sin; return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(
        cos(x.value()), x.derivatives() * (-sin(x.value())));)
...
inline const AutoDiffScalar<VectorXd> pow(const AutoDiffScalar<VectorXd>& a,
  ...
  using std::pow;
  return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(
      pow(a.value(), b), a.derivatives() * (b * pow(a.value(), b - 1)));
}
