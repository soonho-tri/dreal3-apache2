...
class AutoDiffScalar<VectorXd>
 ...
 public:
  ...
  inline const AutoDiffScalar<DerType> operator*(const Scalar& other) const {
    return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(m_value * other, m_derivatives * other);
  }

  friend inline const AutoDiffScalar<DerType> operator*(
      const Scalar& other, const AutoDiffScalar& a) {
    return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(a.value() * other, a.derivatives() * other);
  }

  inline const AutoDiffScalar<DerType> operator/(const Scalar& other) const {
    return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(m_value / other,
...
DRAKE_EIGEN_AUTODIFFXD_DECLARE_GLOBAL_UNARY(
    abs, using std::abs; return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(
        abs(x.value()), x.derivatives() * (x.value() < 0 ? -1 : 1));)

DRAKE_EIGEN_AUTODIFFXD_DECLARE_GLOBAL_UNARY(
    abs2, using numext::abs2; return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(
        abs2(x.value()), x.derivatives() * (Scalar(2) * x.value()));)

DRAKE_EIGEN_AUTODIFFXD_DECLARE_GLOBAL_UNARY(
    sqrt, using std::sqrt; Scalar sqrtx = sqrt(x.value());
    return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(sqrtx,
                                     x.derivatives() * (Scalar(0.5) / sqrtx));)

DRAKE_EIGEN_AUTODIFFXD_DECLARE_GLOBAL_UNARY(
    cos, using std::cos; using std::sin;
    return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(cos(x.value()),
                                     x.derivatives() * (-sin(x.value())));)

DRAKE_EIGEN_AUTODIFFXD_DECLARE_GLOBAL_UNARY(
    sin, using std::sin; using std::cos;
    return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(sin(x.value()),
                                     x.derivatives() * cos(x.value()));)

DRAKE_EIGEN_AUTODIFFXD_DECLARE_GLOBAL_UNARY(
    exp, using std::exp; Scalar expx = exp(x.value());
    return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(expx, x.derivatives() * expx);)

DRAKE_EIGEN_AUTODIFFXD_DECLARE_GLOBAL_UNARY(
    log, using std::log; return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(
        log(x.value()), x.derivatives() * (Scalar(1) / x.value()));)

DRAKE_EIGEN_AUTODIFFXD_DECLARE_GLOBAL_UNARY(
    tan, using std::tan; using std::cos; return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(
        tan(x.value()),
        x.derivatives() * (Scalar(1) / numext::abs2(cos(x.value()))));)

DRAKE_EIGEN_AUTODIFFXD_DECLARE_GLOBAL_UNARY(
    asin, using std::sqrt; using std::asin; return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(
        asin(x.value()),
        x.derivatives() * (Scalar(1) / sqrt(1 - numext::abs2(x.value()))));)

DRAKE_EIGEN_AUTODIFFXD_DECLARE_GLOBAL_UNARY(
    acos, using std::sqrt; using std::acos; return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(
        acos(x.value()),
        x.derivatives() * (Scalar(-1) / sqrt(1 - numext::abs2(x.value()))));)

DRAKE_EIGEN_AUTODIFFXD_DECLARE_GLOBAL_UNARY(
    tanh, using std::cosh; using std::tanh; return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(
        tanh(x.value()),
        x.derivatives() * (Scalar(1) / numext::abs2(cosh(x.value()))));)

DRAKE_EIGEN_AUTODIFFXD_DECLARE_GLOBAL_UNARY(
    sinh, using std::sinh; using std::cosh;
    return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(sinh(x.value()),
                                     x.derivatives() * cosh(x.value()));)

DRAKE_EIGEN_AUTODIFFXD_DECLARE_GLOBAL_UNARY(
    cosh, using std::sinh; using std::cosh;
    return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(cosh(x.value()),
...
inline const AutoDiffScalar<VectorXd> pow(const AutoDiffScalar<VectorXd>& a,
                                          double b) {
  using std::pow;
  return DREAL_EIGEN_MAKE_AUTODIFFSCALAR(pow(a.value(), b),
