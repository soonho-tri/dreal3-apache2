...
BoxCell::BoxCell(const vector<Variable>& variables)
    : variables_{make_shared<vector<Variable>>()},
...
void BoxCell::Add(const Variable& v) {
...
void BoxCell::Add(const Variable& v, const double lb, const double ub) {
  Add(v);
...
bool BoxCell::empty() const { return values_.is_empty(); }

void BoxCell::set_empty() { values_.set_empty(); }

int BoxCell::size() const { return variables_->size(); }

Box::Interval& BoxCell::operator[](const int i) {
  DREAL_ASSERT(i < size());
  return values_[i];
}
Box::Interval& BoxCell::operator[](const Variable& var) {
  return values_[(*var_to_idx_)[var]];
}
const Box::Interval& BoxCell::operator[](const int i) const {
  DREAL_ASSERT(i < size());
  return values_[i];
}
const Box::Interval& BoxCell::operator[](const Variable& var) const {
  return values_[(*var_to_idx_)[var]];
}

const vector<Variable>& BoxCell::variables() const { return *variables_; }

const Variable& BoxCell::variable(const int i) const {
  return (*idx_to_var_)[i];
}

bool BoxCell::has_variable(const Variable& var) const {
  return var_to_idx_->count(var) > 0;
}

int BoxCell::index(const Variable& var) const { return (*var_to_idx_)[var]; }

const Box::IntervalVector& BoxCell::interval_vector() const { return values_; }
Box::IntervalVector& BoxCell::mutable_interval_vector() { return values_; }

pair<double, int> BoxCell::MaxDiam() const {
  double max_diam{0.0};
  int idx{-1};
...
pair<Box, Box> BoxCell::bisect(const int i) const {
  const Variable& var{(*idx_to_var_)[i]};
  if (!values_[i].is_bisectable()) {
    throw DREAL_RUNTIME_ERROR(
        "Variable {} = {} is not bisectable but BoxCell::bisect is called.",
        var, values_[i]);
  }
  switch (var.get_type()) {
...
pair<Box, Box> BoxCell::bisect(const Variable& var) const {
...
pair<Box, Box> BoxCell::bisect_int(const int i) const {
