...
bool ParallelBranch(const ibex::BitSet& bitset, const bool stack_left_box_first,
  ...
  if (branching_point >= 0) {
    ...
    if (*global_stack_size < number_of_jobs) {
...
void Worker(const Contractor& contractor, const Config& config,
  ...
  while ((*found_delta_sat == -1) &&
         (!local_stack.empty() ||
          number_of_boxes->load(std::memory_order_relaxed) > 0)) {
  // DREAL_LOG_DEBUG("IcpParallel::Worker() Loop Head {}, {}",
  // *found_delta_sat,
  //                 *number_of_boxes);

  // if (main_thread) {
  //   std::cerr << "(NB = " << *number_of_boxes
  //             << ", LS = " << local_stack.size()
  //             << ", GS = " << *global_stack_size << ")"
  //             << "\n";
  // }
  ...
  // }
    ...
    if (need_to_pop) {
      if (!local_stack.empty()) {
        ...
        // std::cerr << "L+ ";
      } else if (global_stack_size->load(std::memory_order_relaxed) == 0 ||
        ...
        // std::cerr << "G- ";
        global_stack_size->fetch_sub(1, std::memory_order_relaxed);
      }
    }
    ...
    while (!local_stack.empty() &&
           global_stack_size->load(std::memory_order_relaxed) == 0) {
      global_stack_size->fetch_add(1, std::memory_order_relaxed);
    ...
    // Populating Global...
    if (global_stack_size->load(std::memory_order_relaxed) == 0 &&
    ...
    if (current_box.empty()) {
      ...
      number_of_boxes->fetch_sub(1, std::memory_order_relaxed);
...
bool IcpParallel::CheckSat(const Contractor& contractor,
  ...
  std::cerr << "ICP PARALLEL: # of boxes in total       : " << number_of_boxes
            << "\n";
  std::cerr << "ICP PARALLEL: # of boxes in global stack: " << number_of_boxes
            << "\n";
